...
@Component({
  ...
  template: `
    ...
    <tr [ngClass]="animateOnRouteEnter" *ngFor="let invoice of invoices">
      ...
      <td *ngIf="clientId === null" class="right mobileHide">{{invoice.sentDate | date: dateFormat}}</td>
      <td class="right mobileHide"><span>{{invoice.invoicePaid ? 'Yes' : 'No'}}</span></td>
    ...
    <tr [ngClass]="animateOnRouteEnter" class="nohover" *ngIf="invoices.length !== 0">
      <!--
      <td *ngIf="clientId !== null" class="pl15 strong" colspan="2">
